...
void* getValue(ListType listptr, int n) {
    return listptr->data + (n * listptr->elementSize);
...
void push(ListType listptr, void *item) {
    ...
    if (listptr->size < listptr->capacity) {
        ...
        item, (listptr->elementSize));
        listptr->size++;
    }
}
int is_full(ListType listptr) {
    if (listptr->size >= listptr->capacity) {
        void * temp = malloc(listptr->elementSize* (listptr->capacity + 
        100));
        if (temp != NULL) {
            free(temp);
            return 0;
